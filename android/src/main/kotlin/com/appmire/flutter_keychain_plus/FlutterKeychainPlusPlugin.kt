package com.appmire.flutter_keychain_plus
...
class FlutterKeychainPlusPlugin : FlutterPlugin, MethodCallHandler {

    private var channel: MethodChannel? = null
    private var context: Context? = null
    private val WRAPPED_AES_KEY_ITEM = "W0n5hlJtrAH0K8mIreDGxtG"
    private val channelName = "flutter_keychain_plus"
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        try {
            preferences = binding.applicationContext
                .getSharedPreferences("FlutterKeychainPlus", Context.MODE_PRIVATE)
            encryptor = AesStringEncryptor(
                preferences = preferences,
                keyWrapper = RsaKeyStoreKeyWrapper(binding.applicationContext)
            )
        } catch (e: Exception) {
            Log.e("flutter_keychain_plus", "Could not register plugin", e)
        }
    ...
    override fun onMethodCall(call: MethodCall, result: Result): Unit {
        try {
            ...
            Log.e("flutter_keychain_plus", e.localizedMessage ?: e.toString())
            result.error("flutter_keychain_plus", e.localizedMessage, e)
        }
    }
